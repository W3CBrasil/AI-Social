...
{
 public:
  DenseCrispTube(const dynamic_bitset<>& tube);
  DenseCrispTube(const unsigned int size, const unsigned int presentElement);
 ...
 protected:
  ...
  const unsigned int noiseOnValues(const vector<unsigned int>& valueIds) const;
  ...
  const unsigned int absentFixPresentOrPotentialValuesAfterAbsentValuesMetAndAbsentUsed(Attribute& currentAttribute, vector<unsigned int>& absentValueIntersectionIt) const;
