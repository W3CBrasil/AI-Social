...
# ARGIND == 2 && $6 in article { print $4 "," article[$6] "," 1 }' - "$2" | multidupehack --ids , --ods \; --ha 1 /dev/stdin -o /dev/stdout | d-peeler -m --ids \; --ods " & " --ois ", " --all "Todas as seções" /dev/stdin -o /dev/stdout | ./sort-n-sets | sed 's/$/\\\\\n\\hline/' > biclusters.readable
